"""
Matrix Calculator REST API (FastAPI)
===================================
Single-file, production-ready example implementing a matrix calculator API.

Features:
- Endpoints for: add, subtract, multiply, scalar multiply, transpose,
  determinant, inverse, rank, solve (linear equations)
- Strong input validation using Pydantic
- Clear, consistent JSON responses
- Graceful error handling (with helpful messages)
- Request logging middleware
- Simple in-memory rate limiting (per-IP, sliding window) for basic protection
- Utilities to convert between Python lists and NumPy arrays
...
Notes:
- This implementation uses an in-memory rate limiter which is fine for demos and
  small deployments. For production, use a distributed rate-limiter (Redis, API
  gateway, etc.).
- Numeric operations use NumPy for robustness and performance. Inputs are
  validated and coerced to floats.
- Responses always return JSON with the shape:
  { "success": bool, "result": <value|null>, "error": <string|null> }
